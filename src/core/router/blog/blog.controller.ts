import {Body, Controller, Param, Post, Put, Res, Get, Query} from '@nestjs/common'
...
export class BlogsController {
    ...
    @Get(':pageNumber')
    async getAllByPage(@Query() query: QueryBlog, @Param('pageNumber') pageNumber: number, @Res() res: Response){
        try {
            console.log('Query param', query)
            const {tags, email} = query 
            const listBlog = await this.blogService.getPageBlog(pageNumber, query.take, {tags, email})
            res.json({listBlog})
        } catch (error) {
    ...
    @Put(':id')
    async updateBlog(@Param('id') id: number, @Body() updateBlogDto: UpdateBlogDto , @Res() res: Response){
        try {
