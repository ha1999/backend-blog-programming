...
class FireBase {
  ...
  async uploadFileWithBuffer(
    ...
    return file
      .save(fileUpload, { metadata })
      .then(
        () =>
          `${url_firebase}${bucket_firebase}/o/${fileName.replace(
            /\//g,
            '%2F',
  ...
  async getListFileonFirebase(prefix: string) {
    return bucket.getFiles({ prefix }).then((res) =>
      res[0].map((data) => {
        return `${url_firebase}${
          data.metadata.bucket
        }/o/${data.metadata.name.replace(/\//g, '%2F')}?alt=media&token=${
  ...
  async uploadMultipleFile(listFiles: FileInfo[]) {
    const listPromise = listFiles.map((file) =>
      this.uploadFileWithBuffer(file.originalname, file.buffer, file.mimetype),
    );
